...
// Read files
let a1Data = JSON.parse(fs.readFileSync(a1Path, 'utf8'));
let a2Data = JSON.parse(fs.readFileSync(a2Path, 'utf8'));
let b1Data = JSON.parse(fs.readFileSync(b1Path, 'utf8'));
let b2Data = JSON.parse(fs.readFileSync(b2Path, 'utf8'));
let c1Data = JSON.parse(fs.readFileSync(c1Path, 'utf8'));

// Filter out any previous 'sys-' additions to re-process cleanly
a1Data = a1Data.filter(i => !i.id.startsWith('sys-'));
a2Data = a2Data.filter(i => !i.id.startsWith('sys-'));
b1Data = b1Data.filter(i => !i.id.startsWith('sys-'));
b2Data = b2Data.filter(i => !i.id.startsWith('sys-'));

// Helper to build dedupe key
const getDedupeKey = (level, category, es) => `${level.toUpperCase()}_${category}_${es.toLowerCase().trim()}`;

// Build existing keys set from pre-existing 2000 entries per file
...
console.log(`Initial Base Dedupe Keys count: ${existingKeys.size}`);
...
// CONFIRMED A2 TARGET (Parts 3, 4 + Lesson 21 & Lesson 26)
const a2TargetItems = [
  // Part 3 (Lessons 9-12)
  ...
  { category: 'Adverbios', es: 'fácilmente', en: 'easily', example: 'Resolvemos el problem fácilmente.', exampleTranslation: 'We solve the problem easily.' },
  ...
  // CONFIRMED LESSON 21 MOVE TO A2: Reflexive Verbs & Routine
  ...
  // CONFIRMED LESSON 26 MOVE TO A2: Preterite Past Regulars
...
// CONFIRMED B1 TARGET (Part 5 Lessons 17-20, Part 6 Lessons 22-25 + Lesson 27 & Lesson 29)
const b1TargetItems = [
  // Part 5 (Lessons 17-20)
  ...
  // CONFIRMED LESSON 27 MOVE TO B1: Imperfect Tense
  ...
  // CONFIRMED LESSON 29 MOVE TO B1: Preterite vs Imperfect Contrast
...
// CONFIRMED B2 TARGET (Part 7 Lessons 28 & 30)
const b2TargetItems = [
...
let a2Counter = a2Data.length + 1;
let b1Counter = b1Data.length + 1;
...
console.log('=== CONFIRMED REALIGNMENT SUMMARY ===');
console.log(`A1 additions: ${additions.a1.length} (Total: ${a1Data.length})`);
console.log(`A2 additions: ${additions.a2.length} (Total: ${a2Data.length})`);
console.log(`B1 additions: ${additions.b1.length} (Total: ${b1Data.length})`);
console.log(`B2 additions: ${additions.b2.length} (Total: ${b2Data.length})`);
